...
public class MsgNormal extends Message {
    ...
    public void fromBytes(byte[] binaryMsg) throws Exception {
        ...
        if (!isAck()) {
            ...
            //九、实体数据
            if (bizType != 15) {
                if (!(binaryMsg[_offset] == END_HEAD[0] && binaryMsg[_offset + 1] == END_HEAD[1]))
                    throw new Exception("消息字节串异常！");
                _offset += 4;
    ...
    public byte[] toBytes() throws Exception {//1服务器；0设备
        if (bizType == 15 && msgType == 1 && affirm == 0 && fromType != 1)
            throw new Exception("注册回复消息格式错误!");
        ;
        int _offset = 0;
        ...
        if (!isAck()) {
            //八、用户类型
            if (StringUtils.isNullOrEmptyOrSpace(IMEI) && fromType == 0)
