...
//            cursor = db.rawQuery("Select * from message_notify  where bjuserid=? and showtype=? order by addtime desc", new String[]{userid,"true"});
            cursor = db.rawQuery("Select * from message_notify  where user_id=?  order by add_time desc", new String[]{_user_id});
     ...
     * @param MessageId 消息ID
     */
    public void updataNotifyMessage(String MessageId) {
        SQLiteDatabase db = helper.getWritableDatabase();
        db.execSQL("update message_notify set show_type=? where message_id=?",
