...
/**
 * Activity管理器代码
 * 说明：主要是建立一个栈，把每个已打开的Activity压入栈中。退出的时候在依次取出来
 * 介绍：在每一个activity中的onCreate方法里调用压入方法把当前activity压入管理栈中。
 * 比如在MainActivity中：MyActivityManager mam = MyActivityManager.getInstance();mam.pushOneActivity(MainActivity.this);就把当前activity压入了栈中。
